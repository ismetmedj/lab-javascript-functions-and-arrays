...
//console.log(maxOfTwoNumbers(-43,7));
...
function sum(arrayOfAnyType) {
  ...
  for (i = 0; i < arrayOfAnyType.length; i++) {
    ...
    if (typeof arrayOfAnyType[i] === "number") {
      ...
      sumOfWords += arrayOfAnyType[i].length;
    } else if (typeof arrayOfAnyType[i] === "boolean") {
      if (arrayOfAnyType[i]) {
        sumOfBooleans += 1;
      }
    }
  }
  return sumOfBooleans + sumOfNumbers + sumOfWords;
}
...
function averageWordLength(arrayOfWords) {
  ...
  for (i = 0; i < arrayOfWords.length; i++) {
    averageOfArray += arrayOfWords[i].length;
  }
  return averageOfArray / arrayOfWords.length;
}
  ...
  for (let i = 0; i < arrayOfWords.length; i++) {
    ...
    for (let j = i + 1; j < arrayOfWords.length; j++) {
      if (arrayOfWords[i] === arrayOfWords[j]) {
        unique = false;
        break;
        // const newArray = arrayOfWords.splice(j,1);
        // console.log('nouveau tableau ',newArray);
      }
    }
    if (unique) {
